...
void TriaFinder::reinit(const Triangulation &t) {
  ...
  mapping.reset(new dealii::MappingQGeneric<2>(1));

  v_index.reset(new KDTree(2, *this, 100));
  v_index->buildIndex();

  v_to_c.resize(mesh->n_vertices());

  for (cell = mesh->begin_active(); cell != mesh->end(); ++cell) {
    for (v = 0; v < dealii::GeometryInfo<2>::vertices_per_cell; v++) {
...
TriaFinder::find_active_cell_around_point(const Point &p) const {
  bool inside_cell;
  ...
  while (!q.empty()) {
    ...
    try {
      inside_cell = dealii::GeometryInfo<2>::is_inside_unit_cell(
          mapping->transform_real_to_unit_cell(cell, p), EPS);
    } catch (const dealii::Mapping<2>::ExcTransformationFailed &) {
      inside_cell = false;
    }

    if (inside_cell) {
      cell_point = std::make_pair(cell, dealii::GeometryInfo<2>::project_to_unit_cell(
